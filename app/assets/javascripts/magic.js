...
// Slider noticias index
	...
	photoPosNews = 0;
	photoCntNews = $('#header-inside .container .content .ph').size();
	photoSizeNews = photoCntNews * 610;
 	$('#header-inside .container .content').css('width', photoSizeNews);

 	if(photoCntNews>1){
	 	var myCounter = setInterval(function(){
		 	if(photoPosNews<photoCntNews-1){
			 	photoPosNews+=1;
			 	$('#header-inside .container .content .ph').fadeOut(200, function(){
					$('#header-inside .container .content .ph').eq(photoPosNews).show();
				});
...
// Slider de fotos
		...
		newsSize_News = photoCnt_News * 586;
		...
		$('.prevslider').off('click').on('click', function(event){
			...
			if (photoPos_News > 1){
				photoPos_News--;
				$('.phslider').animate({"left": "+=586px"}, { duration: 500, easing: 'easeOutQuint' });
